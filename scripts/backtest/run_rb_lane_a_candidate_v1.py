#!/usr/bin/env python3
"""One-shot RB Lane A transition-gated allocation evaluation.

This runner wires the already-frozen Gate 0, Amendment-6 same-job authority,
Amendment-8 dual-market translation, candidate mechanism, candidate arms, and
complete preregistered gate suite.  It deliberately separates construction from
outcome scoring: no actual/postgame field is accessed for candidate evaluation until
Gate 0, authority reconstruction, mechanism-comparator integrity, and rush-yard
translation constructibility have all passed for both rotations.

No model fitting, feature search, threshold tuning, sportsbook input, or post-result
rescue path exists here.
"""
...
def construct_rotation_outcome_blind(
    rotation: int,
    root: Path,
    ...
    """Build Lane-A projections without accessing actual outcome columns."""
    test_season = ROTATIONS[rotation]["test_season"]
    ...
    for e in events.itertuples(index=False):
        ...
        # Frozen HHI coordinate contract (head 216cd8d): membership comes from
        # the prior resolvable roster snapshot, but the history cutoff is the
        # transition week itself.
        ...
        active_enriched, hhi_lookup = compute_role_weights_and_hhi(cur, pre, logs)
        allocation, meta = compute_hhi_dampened_reallocation(active_enriched, hhi_lookup, pool_row)
        meta.update({"rotation": rotation, "season": int(e.season), "week": int(e.week), "team": e.team})
        if meta.get("all_zero_weights"):
            meta["conservation_delta"] = None
...
def score_rotation(rotation: int, context: dict, out_dir: Path) -> tuple[dict, pd.DataFrame]:
    """Outcome-touching stage. Called only after both rotations pass constructibility."""
...
def main() -> int:
    ...
    contexts = {
        1: construct_rotation_outcome_blind(1, roots[1], build_a[1], roster, injuries, events),
        2: construct_rotation_outcome_blind(2, roots[2], build_a[2], roster, injuries, events),
    }
    for r in (1, 2):
