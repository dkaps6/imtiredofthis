...
Ported from ``research-rb-m95q-expanded-historical-stable-workhorse``
(commit ``92bd39f6``, blob ``0f8f1fd6a176767d5053c2a940ceea36129084bd``) for RB
Lane A's comparator reconstruction, per GPT-5.6's adjudication (Issue #535
comment `5702639446`) that the canonical M91 promotion-comparator parity check
requires reproducing this exact invocation, not the generic
``historical_player_logs.py`` used by Lane A's first (invalid) reconstruction
attempt. This file was never merged to ``main``; everything below this
docstring (code starting at the ``from __future__`` import) is byte-for-byte
identical to the source blob, verified via diff -- only this provenance
paragraph was added to the module docstring.
