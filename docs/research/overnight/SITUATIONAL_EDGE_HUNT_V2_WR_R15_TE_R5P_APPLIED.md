...
# Situational Edge Hunt V2 — WR-R15/TE-R5P Applied
...
`WR_R15_WR1_ANCHORED_PARTICIPATION` and `TE_R5P_PARTICIPATION_ENTITLEMENT`
models were not yet applied (disclosed gap in
...
- **TE-R5P**: joined `te_r5_oos_player_casebook.csv` (canonical artifact, run
  `34132127351`, artifact `10022512461`), `candidate_receptions_r5`/
  `candidate_rec_yards_r5`, to TE position rows. Full 2023-2025 coverage.
...
- 3,082 rows got a WR-R15 projection (2023-2024 WR receptions/rec_yards);
  2,669 rows got a TE-R5P projection (2023-2025 TE receptions/rec_yards).
...
| Market | Before (base engine only) | After (WR-R15/TE-R5P applied) |
|---|---|---|
...
but real, directionally consistent improvement from applying the actual
promoted models instead of the generic ensemble — as expected, not a
surprise, but good confirmation the entitlement layer is pulling its weight.
...
| | V1 (base engine) | V2 (WR-R15/TE-R5P applied) |
|---|---:|---:|
| n | 1,258 | 1,105 |
...
Splitting the V2 top-quartile/UNDER population by whether WR-R15/TE-R5P
actually changed that row's projection:

| | n | Pooled ROI | Win rate |
|---|---:|---:|---:|
| Touched by WR-R15/TE-R5P | 396 | **+2.94%** | 54.0% |
| Untouched (base engine) | 709 | +0.70% | 52.3% |

The rows the real promoted models touched show a *stronger* edge than the
rows still on the base engine — encouraging, and the direction you'd want.
**But do not over-read this**: broken out by season, the touched group is
2024 n=375 (+4.46% ROI, strong) vs. **2025 n=21 (-24.35% ROI)** — and that
tiny 2025 slice is almost entirely TE-R5P (since WR-R15 has zero 2025
coverage by design). n=21 is nowhere near enough to draw a conclusion either
way; it's a couple of losses away from flipping sign. Treat the "touched
rows are stronger" read as suggestive, not confirmed, until there's a larger
2025 TE-only sample or (if you ever want it) a genuine WR-R15 2025
confirmation pass.

## Bottom line

The V1 candidate is not an artifact of grading receptions on the wrong
model — it survives contact with the real promoted entitlement layer, softer
but still real and still consistent both seasons. The promoted models
