...
def main():
 ...
 integrity={'b0_n_exact':bool(parity['n']==EXPECTED_N),'b0_mae_within_0_05':bool(abs(parity['mae']-EXPECTED_MAE)<=.05),'attempt_source_coverage_ge_0_95':bool(valid_cov>=.95),'team_games_all_accounted':bool(len(teams)>0 and teams.attempt_rate_source.isin(['historical_pregame_pbp','fallback_1.0']).all()),'sportsbook_inputs_used':False};iok=all(bool(v) for k,v in integrity.items() if k!='sportsbook_inputs_used')
 gates={'team_modeled_target_mae_improves_ge_0_50':bool(team_b0['mae']-team_c4['mae']>=.50),'macro_player_target_mae_improves_ge_0_03':bool(ptarget_b-ptarget_c>=.03),'no_position_target_mae_worse_gt_0_03':bool(all(getmae(pm,'POOLED',p,'C4','targets')-getmae(pm,'POOLED',p,'B0','targets')<=.03 for p in POS)),'no_position_reception_mae_worse_gt_0_03':bool(all(getmae(pm,'POOLED',p,'C4','receptions')-getmae(pm,'POOLED',p,'B0','receptions')<=.03 for p in POS)),'no_position_rec_yard_mae_worse_gt_0_50':bool(all(getmae(pm,'POOLED',p,'C4','rec_yards')-getmae(pm,'POOLED',p,'B0','rec_yards')<=.50 for p in POS)),'macro_rec_yard_mae_improves':bool(pyard_c<pyard_b),'macro_rec_yard_improves_ge4_of6_seasons':bool(years>=4),'latest_2024_2025_macro_rec_yard_improves':bool(latest_c<latest_b),'no_season_position_rec_yard_regression_gt_1_50':bool(max(season_pos)<=1.50)};passed=iok and all(gates.values());disp='ATTEMPT_SEMANTICS_CANDIDATE_PASS' if passed else ('BASELINE_OR_INTEGRITY_FAIL' if not iok else 'ATTEMPT_SEMANTICS_CANDIDATE_FAIL')
 result={'migration':'RECEIVING_ATTEMPT_SEMANTICS_V1','seasons':SEASONS,'iterations':2000,'production_changed':False,'sportsbook_inputs_used':False,'baseline_parity':parity,'attempt_source_coverage':valid_cov,'team_target_b0':team_b0,'team_target_c4':team_c4,'macro_player_target_mae_b0':float(ptarget_b),'macro_player_target_mae_c4':float(ptarget_c),'macro_player_rec_yard_mae_b0':float(pyard_b),'macro_player_rec_yard_mae_c4':float(pyard_c),'yearly_macro_rec_yard_improve_count':int(years),'integrity_gates':integrity,'scientific_gates':gates,'disposition':disp};a.out_dir.mkdir(parents=True,exist_ok=True);pm.to_csv(a.out_dir/'attempt_v1_player_metric_summary.csv',index=False);tt.to_csv(a.out_dir/'attempt_v1_team_target_casebook.csv',index=False);teams.to_csv(a.out_dir/'attempt_v1_team_conversion_casebook.csv',index=False);(a.out_dir/'attempt_v1_result.json').write_text(json.dumps(result,indent=2,sort_keys=True)+'\n');print(json.dumps(result,indent=2,sort_keys=True));return 0
