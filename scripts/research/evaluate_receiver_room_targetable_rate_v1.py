#!/usr/bin/env python3
"""Temporal room calibration for Receiver Room Targetable-Rate V1.

Frozen candidate for room g in WR/TE/RB_FB:
  R_g = cumulative strict-prior room targets / cumulative strict-prior team dropbacks
  candidate_room_targets = projected_dropbacks * R_g

2022-2023 baseline uses the historically legitimate pre-specialist/M38 explicit
entitlement stack. No TE-R5P/WR-R15 backcast is allowed.
"""
...
VERSION = "RECEIVER_ROOM_TARGETABLE_RATE_V1"
ROOMS = ("WR", "TE", "RB_FB")
...
def build_room_history(player_logs: pd.DataFrame, team_weekly: pd.DataFrame) -> pd.DataFrame:
    ...
    tg = tw[["season", "week", "team", "actual_dropbacks"]].dropna().copy()
    if (tg["actual_dropbacks"] <= 0).any():
        raise RuntimeError("nonpositive actual dropbacks in history")
    ...
    bad = room_sum.loc[room_sum["modeled_room_targets"] > room_sum["actual_dropbacks"] + 1e-9]
    if not bad.empty:
        raise RuntimeError(
            f"room targets exceed dropbacks sample={bad.head().to_dict('records')}"
        )
...
def strict_prior_room_rates(
    ...
    *,
    ...
    h_games = h[["season", "week", "team", "actual_dropbacks"]].drop_duplicates(
        ["season", "week", "team"]
    )
    lg_drop = float(h_games["actual_dropbacks"].sum())
    if lg_drop <= 0:
        raise RuntimeError("league strict-prior dropbacks <=0")

    league_targets = h.groupby("room")["actual_room_targets"].sum().to_dict()
    league_rates = {r: float(league_targets.get(r, 0.0) / lg_drop) for r in ROOMS}

    team = canon_team(team)
    t = h.loc[h["team"].eq(team)].copy()
    t_games = t[["season", "week", "team", "actual_dropbacks"]].drop_duplicates(
        ["season", "week", "team"]
    )
    tdrop = float(t_games["actual_dropbacks"].sum())
    if tdrop > 0:
        tt = t.groupby("room")["actual_room_targets"].sum().to_dict()
        rates = {r: float(tt.get(r, 0.0) / tdrop) for r in ROOMS}
    ...
    return {
        ...
        "prior_history_dropbacks": hist_drop,
    ...
    for team, tdf in explicit.groupby("team", sort=True):
        ...
        for room_name in ROOMS:
            ...
            rows.append({
                ...
                "room_targetable_rate": rate,
                "candidate_room_targets": projected_dropbacks * rate,
                "conversion_source": info["conversion_source"],
                "prior_history_games": int(info["prior_history_games"]),
                "prior_history_dropbacks": float(info["prior_history_dropbacks"]),
...
def summarize(detail: pd.DataFrame) -> dict:
    ...
    out["pooled"]["min_room_rate"] = float(detail["room_targetable_rate"].min())
    out["pooled"]["max_room_rate"] = float(detail["room_targetable_rate"].max())
    return out
...
def main() -> int:
    ...
    disposition = (
        "RECEIVER_ROOM_TARGETABLE_RATE_V1_SUPPORTED"
        if qualified
        else "RECEIVER_ROOM_TARGETABLE_RATE_V1_FAILED_CLOSED"
    )
    payload = {
        "version": VERSION,
    ...
    lines = [
        "# Receiver Room Targetable-Rate V1 — 2022-2023 Temporal Screen",
        "",
