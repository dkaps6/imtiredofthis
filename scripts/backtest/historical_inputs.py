...
All player universes come from nflverse weekly roster/depth-chart snapshots,
never from target-week box scores. Team-week features are built from historical
PBP and later lagged by ``historical_context.py`` before a prediction week.
"""
...
def build_schedule_history(seasons: Iterable[int]) -> pd.DataFrame:
    ...
    for season in sorted(set(int(s) for s in seasons)):
        sched = _lower(build_or_get_schedule(season))
...
def build_team_weekly_from_pbp(seasons: Iterable[int]) -> pd.DataFrame:
    ...
    for season in sorted(set(int(s) for s in seasons)):
        ...
        league = x.loc[x["off_play"].eq(1)].groupby("week").agg(
            league_pass_rate=("qb_dropback", "mean")
        )
        ...
        for (week, team), g in x.loc[x["off_play"].eq(1) & x["posteam"].ne("")].groupby(["week", "posteam"]):
            ...
            if "game_id" in g.columns and g["game_seconds_remaining"].notna().any():
                ...
                d["prev_clock"] = d.groupby("game_id")["game_seconds_remaining"].shift(1)
                delta = d["prev_clock"] - d["game_seconds_remaining"]
...
def build_pregame_universe_for_week(
    *, season: int, week: int, schedule_history: pd.DataFrame,
    ...
    """Create one historical pregame player universe from roster/depth data.

    No target-week statistical result is read or required.
    """
    sched = _lower(schedule_history)
    sched = sched.loc[pd.to_numeric(sched["season"], errors="coerce").eq(int(season)) & pd.to_numeric(sched["week"], errors="coerce").eq(int(week))]
    if sched.empty:
    ...
    r = r.loc[pd.to_numeric(r["season"], errors="coerce").eq(int(season)) & pd.to_numeric(r["week"], errors="coerce").eq(int(week))].copy()
    ...
    if not d.empty and {"season", "week"}.issubset(d.columns):
        team_col = "club_code" if "club_code" in d.columns else "team" if "team" in d.columns else None
        dname_col = "full_name" if "full_name" in d.columns else None
        if team_col and dname_col:
            d = d.loc[pd.to_numeric(d["season"], errors="coerce").eq(int(season)) & pd.to_numeric(d["week"], errors="coerce").eq(int(week))].copy()
            d["team"] = d[team_col].map(canon_team)
            d["player_join"] = d[dname_col].astype(str).str.lower().str.replace(r"[^a-z0-9]", "", regex=True)
            role_col = "depth_position" if "depth_position" in d.columns else "position" if "position" in d.columns else None
            rank_col = "depth_team" if "depth_team" in d.columns else None
            cols = ["team", "player_join"] + ([role_col] if role_col else []) + ([rank_col] if rank_col else [])
            d = d[cols].drop_duplicates(["team", "player_join"])
        else:
            d = pd.DataFrame()
    ...
    if not d.empty:
        r = r.merge(d, on=["team", "player_join"], how="left")
    role_col = "depth_position" if "depth_position" in r.columns else None
    rank_col = "depth_team" if "depth_team" in r.columns else None

    rows = []
    for _, p in r.iterrows():
        opp, game_id = matchup[p["team"]]
        role = str(p.get(role_col, "") or "") if role_col else ""
        if rank_col and pd.notna(p.get(rank_col)):
            role = f"{role}{str(p.get(rank_col)).strip()}" if role else f"DEPTH{str(p.get(rank_col)).strip()}"
        rows.append({
            "player": p["player"], "team": p["team"], "opponent": opp,
            "position": p["position"], "role": role, "game_id": game_id,
            "season": int(season), "week": int(week),
            "pregame_source": "nflverse_weekly_roster+depth_chart",
        })
