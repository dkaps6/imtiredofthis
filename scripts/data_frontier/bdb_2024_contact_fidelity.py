...
def _numeric_summary(series: pd.Series) -> dict:
    ...
    return {"n": int(len(x)), "missing": int(len(series) - len(x)), "min": float(x.min()), "p05": float(x.quantile(0.05)), "median": float(x.median()), "p95": float(x.quantile(0.95)), "max": float(x.max())}


def build_fidelity_report(enriched: pd.DataFrame, defenders: pd.DataFrame, dispositions: pd.DataFrame, qa_summary: dict, artifact_set_sha256: str | None = None) -> dict:
    ...
    reason_counts = (dispositions.loc[dispositions.get("benchmark_disposition", pd.Series(index=dispositions.index, dtype=object)).ne("SCOREABLE"), reason_col].fillna("<missing>").astype(str).value_counts().sort_index() if reason_col else pd.Series(dtype=int))
    ...
    overlap_cols = ["first_contact_primary_tackle_overlap", "first_contact_assist_overlap", "first_contact_missed_tackle_overlap", "first_contact_any_source_label_overlap"]
    overlaps = {c: _rate(enriched[c]) if c in enriched.columns else None for c in overlap_cols}
    numeric_cols = ["firstContactCarrierSidelineDistanceYards", "firstContactMaxClosingSpeedProxyYdsPerSec", "firstContactMeanClosingSpeedProxyYdsPerSec", "firstContactMinPursuitAngleErrorDeg", "firstContactMeanPursuitAngleErrorDeg"]
    ...
    return {
        ...
        "upstream_artifact_set_sha256": artifact_set_sha256,
        ...
        "geometry_resolution": {"plays_with_one_or_more_resolved_contact_defenders": int((geometry_resolved > 0).sum()), "rate_over_enriched_plays": float((geometry_resolved > 0).mean()) if len(geometry_resolved) else None, "resolved_defender_rows": int(len(defenders))},
        "source_label_overlap_rates": overlaps,
        "geometry_distribution_sanity": distributions,
        "source_window_diagnostics": qa_summary.get("source_window_diagnostics", {}),
        "interpretation_guardrails": ["Overlap with tackle/assist/missed-tackle labels is reconciliation, not predictive accuracy.", "BDB 2024 tracking is event-window filtered; absent events do not prove absent football events.", "Geometry missingness is reported rather than imputed.", "No detector threshold or feature definition is tuned from this report."],
    }
...
def main() -> int:
    ...
    args = ap.parse_args()
    provenance_path = args.artifact_dir / "artifact_provenance_v1.json"
    if not provenance_path.is_file():
        print(json.dumps({"passed": False, "failure": "provenance manifest missing; fidelity refuses unsealed inputs"}, sort_keys=True))
        return 2
    provenance = json.loads(provenance_path.read_text(encoding="utf-8"))
    artifact_set_sha256 = provenance.get("artifact_set_sha256")
    if not artifact_set_sha256:
        print(json.dumps({"passed": False, "failure": "provenance artifact_set_sha256 missing"}, sort_keys=True))
        return 2
    ...
    report = build_fidelity_report(enriched, defenders, dispositions, qa_summary, artifact_set_sha256)
    out = args.artifact_dir / "contact_fidelity_report_v1.json"
    out.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    print(json.dumps({"report_version": REPORT_VERSION, "output": str(out), "attempted_plays": report["attempted_plays"], "scoreable_plays": report["scoreable_plays"], "artifact_set_sha256": artifact_set_sha256, "contact_detector_changed": False}, sort_keys=True))
    return 0
