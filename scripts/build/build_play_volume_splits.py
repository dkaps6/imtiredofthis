...
def fetch_pbp_2025() -> pd.DataFrame:
    ...
    for url in PBP_URLS:
        try:
            ...
            try:
                df = pd.read_csv(buf, compression="gzip", low_memory=False)
            except Exception:
                buf.seek(0)
...
def compute_seconds_per_play(off_df: pd.DataFrame) -> float:
    ...
    dd = dd[(dd["delta"].notna()) & (dd["delta"]>=0) & (dd["delta"]<600)]  # guardrails
    if dd.empty:
        return np.nan
...
def main(out_csv: str="play_volume_splits.csv"):
    ...
    needed = ["week","qtr","score_differential","posteam","defteam","qb_dropback","rush_attempt","game_id","play_id","game_seconds_remaining"]
    for c in needed:
    ...
    pbp["is_off_play"] = ((pbp["qb_dropback"]==1) | (pbp["rush_attempt"]==1)).astype(int)

    # Compute per team-week
    rows = []
    weeks = sorted(pbp["week"].dropna().unique().tolist())
    teams = sorted(set(pbp["posteam"].dropna().unique().tolist()) | set(pbp["defteam"].dropna().unique().tolist()))
    for wk in weeks:
        ...
        for tm in teams:
            ...
            if plays_offense > 0:
                neutral_mask = (off["score_differential"].abs() <= 7) & (off["qtr"].isin([1,2,3]))
            ...
            rows.append({
                ...
                "neutral_situation_rate": None if pd.isna(neutral_rate) else round(neutral_rate,4),
            })

    out = pd.DataFrame(rows, columns=["team","week","plays_offense","plays_defense","seconds_per_play","neutral_situation_rate"])
