...
def resolve_current_roles_path(*, require_active: bool = False) -> Path:
    token = os.getenv("ACTIVE_ROLES_CSV", "").strip()
    if token:
        path = Path(token)
    elif require_active:
    ...
    if not path.exists() or path.stat().st_size <= 0:
        kind = "active reconciled roles" if require_active or token else "raw Ourlads roles"
        raise RuntimeError(f"{kind} missing/empty: {path}")
    return path


def load_current_roles(*, require_active: bool = False) -> pd.DataFrame:
    path = resolve_current_roles_path(require_active=require_active)
    out = pd.read_csv(path, low_memory=False)
    out.columns = [str(c).strip().lower() for c in out.columns]
    required = {"team", "player", "role", "position"}
    missing = required - set(out.columns)
    if missing:
        raise RuntimeError(f"current-role artifact missing {sorted(missing)}: {path}")
    if "player_clean_key" not in out.columns and "player_key" not in out.columns:
        raise RuntimeError(f"current-role artifact missing player identity key: {path}")
    if out.empty:
        raise RuntimeError(f"current-role artifact has zero rows: {path}")
    return out
