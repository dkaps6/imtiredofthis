...
def main() -> int:
    ...
    if text.count(OLD) != 1:
        raise RuntimeError(f"QB C2 primary coverage anchor count={text.count(OLD)}")
    out = text.replace(OLD, NEW)
    if 'QB C2 state context must cover exactly 32 teams' not in out:
        raise RuntimeError("QB C2 state-context 32-team source guard changed unexpectedly")
    PATH.write_text(out, encoding="utf-8")
    print("CURRENT_AVAILABILITY_QB_C2_PRIMARY_TEAM_SEAM_TRANSFORM_PASS")
    return 0
