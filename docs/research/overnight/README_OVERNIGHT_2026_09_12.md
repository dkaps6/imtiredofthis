...
**V2 update**: that V1 result graded receptions on the base engine only (disclosed gap — WR-R15/TE-R5P not yet applied). You located and provided two canonical GH Actions validation artifacts (`wr_r15_confirmation_predictions.csv` for WR-R15, and `te_r5_oos_player_casebook.csv` — direct download was blocked by this session's network egress policy, an org-level block on Azure blob storage, not a repo issue) and I applied them to the same cohort and re-graded with the unmodified grading script. **Correction (Issue #535 checkpoint 28)**: the second artifact is **TE-R5**, an earlier scientific-decomposition model (run `34132127351`/artifact `10022512461`), not the later production-certified **TE-R5P** (run `34152797603`/artifact `10029942404`) — this doc and `SITUATIONAL_EDGE_HUNT_V2_WR_R15_TE_R5P_APPLIED.md` originally mislabeled it; a genuine TE-R5P replay is separate, not-yet-done follow-up work (PR #549). The numbers below are unchanged and correct for WR-R15 + TE-R5 as actually applied. **The edge survives**: softens from +2.66% to +1.50% pooled ROI but stays positive independently in both seasons (2024 +2.07%, 2025 +0.92%). The subset of bets these two models actually touched shows a stronger edge (+2.94%) than the untouched subset (+0.70%) — encouraging, but the 2025-specific slice of that (n=21, almost all TE-R5 since WR-R15 has zero 2025 coverage by its own frozen contract) is too small to lean on. Full writeup in `SITUATIONAL_EDGE_HUNT_V2_WR_R15_TE_R5P_APPLIED.md`.
