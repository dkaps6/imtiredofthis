...
def main():
    q=argparse.ArgumentParser();q.add_argument('--participation-trace',type=Path,default=Path('data/backtests/qb_participation_role/qb_participation_player_trace.csv'));q.add_argument('--injuries',type=Path,default=Path('data/backtests/injuries_history.csv'));q.add_argument('--universe-dir',type=Path,default=Path('data/backtests/pregame_universe'));q.add_argument('--out-dir',type=Path,default=Path('data/backtests/qb_pregame_availability'));a=q.parse_args();x=read(a.participation_trace);inj=read(a.injuries) if a.injuries.exists() else pd.DataFrame()
    ...
    for _,r in x.iterrows():
        ...
        share=float(num(pd.Series([r.get('actual_qb_attempt_share',r.get('actual_qb_share'))])).fillna(0).iloc[0]);pclass=str(r.get('participation_class',''))
        mismatch=bool(r.get('role_mismatch',False)) or (pclass not in {'stable_primary_80plus','primary_but_shared'} and share<.8)
    ...
    for name,g in [('all',z),('stable_or_shared',z[z.audit_class=='stable_or_shared']),('pregame_flagged_disruption',z[z.audit_class=='pregame_flagged_disruption']),('unflagged_disruption',z[z.audit_class=='unflagged_disruption'])]:
        if g.empty: continue
        d={'scope':name,'games':len(g),'mean_actual_qb_share':num(g.get('actual_qb_attempt_share',g.get('actual_qb_share'))).mean(),'mean_actual_attempts':num(g.get('actual_pass_att')).mean(),'mean_pred_attempts':num(g.get('pred_attempts')).mean(),'risk_rate':num(g.get('pregame_risk_flag')).mean()};d.update(met(g.actual_pass_yards_raw,g.mc_proj));summ.append(d)
