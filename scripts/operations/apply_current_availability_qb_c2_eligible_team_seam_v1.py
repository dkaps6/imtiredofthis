...
def main() -> int:
    ...
    if OLD not in text:
        raise RuntimeError("protected QB C2 legacy coverage anchor not found exactly once")
    if text.count(OLD) != 1:
        raise RuntimeError(f"protected QB C2 coverage anchor count={text.count(OLD)}")
    out = text.replace(OLD, NEW)
    # Explicitly prove the source-context 32-team guard remains untouched.
    ...
    print("CURRENT_AVAILABILITY_QB_C2_ELIGIBLE_TEAM_SEAM_TRANSFORM_PASS")
    return 0
