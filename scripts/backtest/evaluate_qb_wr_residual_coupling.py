...
def main():
 ...
 wr=pd.DataFrame(rows);x=qb.merge(w,on=['season','week','team'],how='inner',validate='one_to_one');scores=pd.DataFrame([score(x,c) for c in ['wr_all_residual','wr_top2_projected_residual','wr_top1_projected_residual']]);r=scores.loc[scores.signal.eq('wr_all_residual')].iloc[0]
 gates={'aligned_ge700':len(x)>=700,'pearson_ge_0_35':r.pearson>=.35,'spearman_ge_0_30':r.spearman>=.30,'same_sign_ge_0_60':r.same_sign_rate>=.60,'quartile_gap_ge30':r.q4_minus_q1_qb_residual_gap>=30,'positive_both_seasons':r.pearson_2024>0 and r.pearson_2025>0};disp='STRONG_QB_WR_ERROR_COUPLING' if all(gates.values()) else 'NO_STRONG_QB_WR_ERROR_COUPLING'
 out={'migration':'QB_WR_RESIDUAL_COUPLING','qb_rows':len(qb),'wr_team_week_rows':len(wr),'aligned_rows':len(x),'gates':gates,'sportsbook_inputs_used':False,'production_changed':False,'m72_status':'aggregate pregame explosive-weapon x defense signal screen previously failed','disposition':disp}
