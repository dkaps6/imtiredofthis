...
def _standardise(df: pd.DataFrame) -> pd.DataFrame:
    ...
    player_col = _first_present(["player","player_display_name","player_name","full_name","gsis_name"], df)
    team_col   = _first_present(["team","recent_team","team_abbr","club","team_code","abbr"], df)
    status_col = _first_present(["status","game_status","injury_status","practice_status","practice_participation"], df)

    if player_col is None:
        return pd.DataFrame(columns=["player", "team", "status"])

    out = pd.DataFrame({"player": df[player_col].astype(str).str.strip()})
    out["team"] = (df[team_col].astype(str).str.upper().str.strip() if team_col else "")
    out["status"] = (df[status_col].astype(str).str.title().str.strip() if status_col else "Unknown")
...
def _write_stub() -> None:
    ...
    pd.DataFrame(columns=["player", "team", "status", "report_date"]).to_csv(OUTPATH, index=False)
    print(f"[injuries] wrote empty stub → {OUTPATH}")

def main(season: int) -> int:
    DATA_DIR.mkdir(exist_ok=True)
    sources: list[str] = []
    df = pd.DataFrame(); used_source = ""

    try:
        import nfl_data_py as nfl
        temp = nfl.import_injuries([season])
        if temp is not None and not temp.empty:
            df = temp; used_source = "nfl_data_py.import_injuries"
    ...
    if df.empty:
        try:
            ...
            if temp is not None and not temp.empty:
                df = temp; used_source = "nflreadpy.load_injuries"
        except Exception as exc:
    ...
    if df.empty:
        ...
        _write_stub(); return 0

    normalised = _standardise(df).drop_duplicates()
    if normalised.empty:
        if sources: print(f"[injuries] normalisation produced no rows ({'; '.join(sources)})")
        _write_stub(); return 0
