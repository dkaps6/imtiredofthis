...
def test_prior_blend_is_explicit_and_current_takes_over():
    prior = pd.DataFrame([{ "player_clean_key": "abc", "games": 17, "tgt_share": .20, "rush_share": 0.0, "route_rate": np.nan, "yprr": np.nan, "ypt": 8.0, "ypc": np.nan, "ypa": np.nan, "receptions_per_target": .65 }])
    current = pd.DataFrame([{ "player_clean_key": "abc", "games": 4, "tgt_share": .30, "rush_share": 0.0, "route_rate": np.nan, "yprr": np.nan, "ypt": 10.0, "ypc": np.nan, "ypa": np.nan, "receptions_per_target": .70 }])
    universe = pd.DataFrame([{ "player": "A B", "player_clean_key": "abc", "team": "BUF", "opponent": "NYJ", "season": 2026, "week": 5, "position": "WR", "role": "WR1" }])
    out = _blend(prior, current, universe)
    # current weight = 4 / (4 + 4) = .5
...
def test_season_totals_yprr_requires_real_routes():
    logs = pd.DataFrame([
        {"season": 2025, "week": 1, "player_clean_key": "abc", "player": "A B", "team": "BUF", "position": "WR", "targets": 10, "receptions": 7, "rec_yards": 100, "rushes": 0, "rush_yards": 0, "pass_att": 0, "pass_yards": 0, "routes": np.nan, "team_targets": 30, "team_rushes": 20, "team_dropbacks": 35, "team_routes": np.nan},
    ])
