...
def certify(schedule:pd.DataFrame, official:pd.DataFrame, *, asof_utc)->tuple[pd.DataFrame,dict]:
    ...
    s=schedule.copy(); s.columns=[str(c).lower() for c in s.columns]; rename={}
    if "home" in s and "home_team" not in s: rename["home"]="home_team"
    if "away" in s and "away_team" not in s: rename["away"]="away_team"
    s=s.rename(columns=rename); req={"season","week","home_team","away_team","kickoff_utc"}; miss=req-set(s.columns)
    ...
    o=official.copy() if official is not None else pd.DataFrame(); complete=set(); snapshot_by_team={}
    if not o.empty:
        ...
        o["source_asof_utc"]=pd.to_datetime(o.source_asof_utc,utc=True,errors="coerce"); good=pd.to_numeric(o.section_complete,errors="coerce").fillna(0).eq(1); complete=set(o.loc[good,"team"])
        for team,g in o[good].groupby("team"):
            vals=g.source_asof_utc.dropna(); snapshot_by_team[team]=vals.max() if len(vals) else pd.NaT
    rows=[]; withheld=[]
    for idx,r in s.reset_index(drop=True).iterrows():
        ko=r.kickoff_utc; mins=(ko-a).total_seconds()/60.0; hc=r.home_team in complete; ac=r.away_team in complete; hts=snapshot_by_team.get(r.home_team,pd.NaT); ats=snapshot_by_team.get(r.away_team,pd.NaT); snap=max([x for x in [hts,ats] if not pd.isna(x)],default=pd.NaT); reasons=[]
        if mins<=0: state="KICKED_OFF_LOCKED"; eligible=False; reasons.append("kickoff_at_or_before_asof")
        elif mins>REQUIRE_MINUTES: state="NOT_YET_REQUIRED"; eligible=True
        else:
            ...
            eligible=not reasons; state="REQUIRED_AND_CERTIFIED" if eligible else "REQUIRED_MISSING_FAIL_CLOSED"
    ...
    out=pd.DataFrame(rows); counts=out.certification_state.value_counts().to_dict() if len(out) else {}
...
def main()->int:
    ...
    official=pd.read_csv(a.official) if a.official.exists() and a.official.stat().st_size else pd.DataFrame(); asof=a.asof_utc or datetime.now(timezone.utc).isoformat(); out,meta=certify(sched,official,asof_utc=asof)
