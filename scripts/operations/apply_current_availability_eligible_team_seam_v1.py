...
This transformer is intentionally surgical. It changes only two legacy current-
coverage assertions identified in the frozen plan. It refuses to operate unless
all exact source anchors are present once and refuses to touch model parameters,
assets, entitlement math, or sportsbook behavior.
"""
...
FULL_EVENT_NEW = '''    form["event_id"] = [\n        _canonical_game(t, o, s, w)\n        for t, o, s, w in zip(form["team"], form["opponent"], form["season"], form["week"])\n    ]\n    # Every certified-eligible team's canonical game is guaranteed present once\n    # validate_current_team_set (above) confirms the team itself is present --\n    # event_id is derived row-wise from team/opponent/season/week for every row\n    # in form. The football simulation is contracted to cover every rostered\n    # team/game regardless of kickoff-timing eligibility, so it may legitimately\n    # observe MORE games than are currently eligible; only fewer is a real bug.\n    expected_games = int(team_coverage.get("canonical_games", 16))\n    observed_games = int(form["event_id"].nunique())\n    if observed_games < expected_games:\n        raise RuntimeError(\n            f"football simulation canonical-game coverage invalid expected>={expected_games} observed={observed_games}"\n        )\n    form["market"] = "football_universe"\n'''
...
def main() -> int:
    ...
    # Explicitly prove legacy guards are gone only in the transformed text.
