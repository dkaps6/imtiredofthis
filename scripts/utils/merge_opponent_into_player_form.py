...
# Bail out cleanly if inputs are missing/empty so CI never hard-fails
...
pf["player_key"] = norm_series(pf["player"]) if "player" in pf.columns else pf.assign(player_key="")
om["player_key"] = norm_series(om["player"]) if "player" in om.columns else om.assign(player_key="")

# Merge preference: (player, team, week) -> (player, week) fallback
left = pf
