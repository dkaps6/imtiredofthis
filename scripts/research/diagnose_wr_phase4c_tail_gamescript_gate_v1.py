...
def _boot_prevalence_diff(df, cohort_col, stratum_mask, reps=BOOT_REPS, seed=BOOT_SEED):
    ...
    base = float(c.mean())
    prev = float(c.loc[s].mean())
    diff = prev - base
    ratio = prev / base if base > 0 else np.nan
    boots = []
    for idx in _cluster_resamples(df, reps, seed):
        b = df.iloc[idx]
...
def _player_rows(l1: pd.DataFrame, reps: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for name, n in EXPECTED_COUNTS.items():
        ...
        if got != n:
            raise RuntimeError(f"cohort drift {name}: {got} != {n}")
    x = l1.copy()
    for name, m in masks.items():
        x[name] = m.astype(bool)
    rows = []
    ctrls = []
    seed_offset = 0
    for scope, g in _season_scopes(x):
        for cohort in EVENT_COHORTS:
            n = int(g[cohort].sum())
            base = float(g[cohort].mean())
            for label in TOTAL_BIN_LABELS:
                sm = g["total_bucket"].eq(label)
                prev, basep, diff, ratio, lo, hi = _boot_prevalence_diff(g, cohort, sm, reps, BOOT_SEED + seed_offset)
                seed_offset += 1
            ...
            for col in CONT:
                est, lo, hi = _boot_continuous_diff(g, cohort, col, None, reps, BOOT_SEED + seed_offset)
                seed_offset += 1
            ...
            if cohort in {"UNDERPROJECT_30_PLUS", "UNDERPROJECT_30_PLUS_OPP_DOM"}:
                for col in CONT:
                    est, lo, hi = _boot_continuous_diff(g, cohort, col, "OVERPROJECT_30_PLUS", reps, BOOT_SEED + seed_offset)
                    seed_offset += 1
...
def _layer2_rows(l23: pd.DataFrame, reps: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    x = l23.copy()
    x["abs_team_pool_component"] = x["team_pool_component"].abs()
    x["direct_team_target_residual"] = x["actual_team_targets"] - x["implied_team_target_pool"]
    outcomes = ["team_pool_component", "abs_team_pool_component", "direct_team_target_residual"]
    corr = []
    buckets = []
    seed = BOOT_SEED + 50000
    for scope, g in _season_scopes(x):
        for concept in CONT:
            for outcome in outcomes:
                rho, lo, hi = _bootstrap_spearman(g, concept, outcome, reps, seed)
                seed += 1
...
def _ci_excludes_zero(r):
    return bool((r["ci_low"] > 0 and r["ci_high"] > 0) or (r["ci_low"] < 0 and r["ci_high"] < 0))


def _evaluate_gate(player: pd.DataFrame, l2corr: pd.DataFrame, l2bucket: pd.DataFrame) -> dict:
    # Bracketed column access is deliberate here: `view` is also a pandas
    # method name, so attribute access would silently bind to the method.
    ...
    for _, r in candidates.iterrows():
        ...
        seas = player[
            (player["cohort"] == PRIMARY)
            & (player["family"] == family)
            & (player["view"] == view)
            & (player["scope"].isin(["2023", "2024"]))
        ]
        season_ok = len(seas) == 2 and all(_sign(float(v)) == sign for v in seas["estimate"])
        rep_rows = player[
            (player["scope"] == "POOLED")
            & (player["cohort"].isin(SECONDARY))
            & (player["family"] == family)
            & (player["view"] == view)
        ]
        replicate_ok = any(_sign(float(v)) == sign for v in rep_rows["estimate"] if np.isfinite(v))
        if family == "continuous":
            z = l2corr[(l2corr["scope"] == "POOLED") & (l2corr["concept"] == concept)]
        else:
            z = l2bucket[
                (l2bucket["scope"] == "POOLED")
                & (l2bucket["family"] == family)
                & (l2bucket["view"] == view)
            ]
        ...
        tested.append(dict(
            ...
            primary_ci_low=float(r["ci_low"]),
            primary_ci_high=float(r["ci_high"]),
            direction=sign,
            season_coherence=season_ok,
            layer2_coherence=layer2_ok,
            secondary_replication=replicate_ok,
            advances=bool(season_ok and layer2_ok and replicate_ok),
            layer2_matching_rows=int(len(coherent)),
        ))
    adv = [x for x in tested if x["advances"]]
    return {
        "disposition": "ADVANCE_TO_SCRIPT_PREDICTOR_DESIGN" if adv else "NO_ACTIONABLE_TAIL_GAMESCRIPT_BRIDGE",
        "primary_nonnull_candidates": len(tested),
        "advancing_candidates": adv,
        "candidate_audit": tested,
    }
...
def _add_market(df, market):
    ...
    out = x.merge(
        market[TG + ["market_total", "market_team_spread", "market_abs_spread", "market_team_implied"]],
        on=TG,
        how="left",
...
def main():
    ...
    ap.add_argument("--layer1", type=Path, required=True)
    ap.add_argument("--layer23", type=Path, required=True)
    ap.add_argument("--out-dir", type=Path, required=True)
    ap.add_argument("--bootstrap-reps", type=int, default=BOOT_REPS)
    a = ap.parse_args()
    l1 = pd.read_csv(a.layer1)
    l23 = pd.read_csv(a.layer23)
    if len(l1) != EXPECTED_L1:
        raise RuntimeError(f"Layer1 row drift {len(l1)}")
    if len(l23) != EXPECTED_L23:
        raise RuntimeError(f"Layer23 row drift {len(l23)}")
    seasons = sorted(set(l1["season"].astype(int)) | set(l23["season"].astype(int)))
    if seasons != [2023, 2024]:
        raise RuntimeError(f"season drift {seasons}")
    market = load_market_schedule(seasons)
    l1 = _add_market(l1, market)
    l23 = _add_market(l23, market)
    p, ctrl = _player_rows(l1, a.bootstrap_reps)
    c, b = _layer2_rows(l23, a.bootstrap_reps)
    gate = _evaluate_gate(p, c, b)
    a.out_dir.mkdir(parents=True, exist_ok=True)
    p.to_csv(a.out_dir / "phase4c_player_tail_market_associations.csv", index=False)
    ctrl.to_csv(a.out_dir / "phase4c_directional_control_associations.csv", index=False)
    c.to_csv(a.out_dir / "phase4c_layer2_market_spearman.csv", index=False)
    b.to_csv(a.out_dir / "phase4c_layer2_market_buckets.csv", index=False)
    meta = {
        "specification": "WR_PHASE4C_TAIL_GAMESCRIPT_GATE_V1",
        "phase4b_layer1_rows": len(l1),
        "phase4b_layer23_team_games": len(l23),
        "cluster_unit": "season-week-team",
        "bootstrap_reps": int(a.bootstrap_reps),
        "comparison_space": {"pooled_cells": 112, "pooled_plus_season_cells": 336, "directional_control_cells": 24},
        "market_lineage": {"implied_team_target_pool_has_market_input": False, "r15_sportsbook_inputs_used": False},
        "gate": gate,
        "production_change": False,
        "script_predictor_built": False,
        "paid_full_slate": False,
        "rb_work": False,
    }
