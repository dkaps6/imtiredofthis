...
Important model-stack contract: the PlayerForm prior/current split is preserved.
Only helper columns created by this scoring enrichment are removed. This keeps
pregame prior-season and current-season evidence available to the canonical
...
def enrich(season: int, prior_season: int, week: int) -> pd.DataFrame:
    ...
    player_evidence_cols = [
        f"{metric}_{era}"
        for metric in ("tgt_share", "rush_share", "route_rate", "yprr", "ypt", "ypc", "ypa", "receptions_per_target")
    ...
    # Remove only the helper columns introduced above. The previous implementation
    # dropped *every* *_prior/*_current column, unintentionally deleting the raw
    # evidence required by Bayesian modeling and historical backtesting.
    scoring_helper_cols = [
...
def main() -> int:
    ...
    print(f"[player_scoring] enriched {len(out)} PlayerForm rows season={season} week={week}; preserved prior/current evidence")
    return 0
